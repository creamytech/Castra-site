...
export const POST = withAuth(async ({ req, ctx }, { params }: any) => {
  try {
    ...
    const deal = await prisma.deal.create({ data: { userId: ctx.session.user.id, orgId: ctx.orgId, title: subject, type, stage: 'LEAD', nextAction: 'Reply to lead', nextDue: new Date() } })
    ...
    const contact = await prisma.contact.create({ data: { userId: ctx.session.user.id, orgId: ctx.orgId, firstName, lastName, email, tags: ['email'] } })
