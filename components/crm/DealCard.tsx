  ...
  return (
    <div className="p-3 rounded-lg border border-border bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-pink-500/10 hover:from-indigo-500/20 hover:via-purple-500/20 hover:to-pink-500/20 transition-colors cursor-pointer touch-manipulation select-none" onClick={() => onOpen ? onOpen(deal.id) : router.push(`/crm/deals/${deal.id}`)}>
      ...
      <div className="mt-2 flex items-center gap-2 flex-wrap">
        ...
        <button disabled={deal.stage === 'CLOSED' || deal.stage === 'LOST'} onClick={async (e)=>{ e.stopPropagation(); const extra = await promptClose(nextStage); if (extra === null) return; await fetch(`/api/deals/${deal.id}/stage`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ stage: nextStage, ...extra }) }); if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('deals:refresh')) }} className="text-xs px-2 py-1 rounded bg-primary text-primary-foreground hover:bg-primary/90 inline-flex items-center gap-1"><ArrowRight className="w-3 h-3" /> {nextStage}</button>
